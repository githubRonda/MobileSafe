...
1. 获取缓存大小
	    ...
	    - 查找源码逻辑
		setting中应用信息界面显示有每个app缓存的大小，所以我们可以查看系统设置源码。
		具体步骤：搜索“清除缓存” --> 搜索 "clear_cache_btn_text" --> 找到 installed_app_details.xml 布局文件，同时记录该btn控件上方的显示缓存大小的TextView的id(cache_size_text) --> 搜索 installed_app_details -->
		找到 InstalledAppDetails.java ，同时搜索 cache_size_text 对应的TextView控件，找到给该控件setText()的地方，发现是这个地方： mCacheSize.setText(getSizeStr(mAppEntry.cacheSize)); 这时要注意：给 mAppEntry.cacheSize 赋值的地方不是在当前类中，而是跟踪过去所在的类中，最终是在创建 mStatsObserver 对象时的内部类中 --> 
		接着查找 mStatsObserver 回调内部方法的时机是在 调用 PackageManager.getPackageSizeInfo() 时触发的。
	
		- 反射调用 PackageManager中的隐藏方法getPackageSizeInfo
...
5. 选项卡介绍
		...
		<?xml version="1.0" encoding="utf-8"?>
		<!-- 放置选项卡和选项卡指向界面的控件
		 要注意：tabhost 、tabcontent、tabs 这三个 id 都是固定的
		 -->
		<TabHost
...
## 流量统计
	...
	- 流量分为：上传(tx) 和 下载(rx) [和串口的命名类似]
	- 连接真机,查看文件/proc/uid_stat,发现该目录下有很多以uid命名的文件夹。这个uid，也叫用户id,是安装应用程序的时候,操作系统为每一个应用赋值的唯一标识
	- 获取uid方式
	...
	- 流量统计的api介绍
			...
			TrafficStats.getMobileRxBytes();// 4g/3g/2g下载总流量
			TrafficStats.getMobileTxBytes();// 4g/3g/2g上传总流量
			...
			这里需要注意的是，
			1. 通过 TrafficStats 获取的数据在手机重启的时候会被清空，所以，如果要对流量进行持续的统计需要将数据保存到数据库中，在手机重启时将数据读出进行累加即可
			2. 以上获取流量的方式意义不大，因为真正决定使用流量多少是由运营商来决定的。所以，应该发送短信给运营商来获取真实的流量使用量， 或者使用 运营商提供的第三方接口来进行流量监听
